...
/*  Galois Field (2^8) - Rijndael Substitution Box. More readable */
...
void RotateWord(unsigned char word[NUM_BLOCKS])
{
	unsigned char tmp;
	/*  eg.
	*   |1F|    |AC|
...
*   NOTE: this function is very similar to SubBytes. They were split for clarity.
*/
void SubstituteWord(unsigned char word[NUM_BLOCKS]) 
{
	unsigned char c;
...
*/
void RCon(unsigned char RCon_column[NUM_BLOCKS], const int round)
{
	/*  eg.
	*   |8c|   |01|   |8d|
    *   |75| + |00| = |75|
...
//  FUNCTIONS - AES
/*------------------------------------------------------------------------------------------------------------------------------*/
void aes256::Encrypt(const unsigned char state_in[TEXT_SIZE], unsigned char state_out[NUM_BLOCKS][NUM_BLOCKS])
{
	/*  State matrix. */
    unsigned char state[NUM_BLOCKS][NUM_BLOCKS];
    /*  RoundKey matrix. */
    unsigned char RoundKey[NUM_BLOCKS][NUM_BLOCKS];
    /*  ExpandedKey matrix. */
    unsigned char ExpandedKey[NUM_BLOCKS][NUM_BLOCKS * (NUM_ROUNDS + 1)];
    
    /*  Initializing state matrix... */
    for(int c = 0, i = 0; c < NUM_BLOCKS; ++c)
    {
    	for(int r = 0; r < NUM_BLOCKS; ++r, ++i)
    		state[r][c] = state_in[i];
	}
	
	aes256::KeyExpansion(ExpandedKey);
	
    /*  AESKeyScheduler set appropriate key for that round in RoundKey. 
	*   First round.
	*/
    aes256::AESKeyScheduler(ExpandedKey, RoundKey, 0);
    aes256::AddRoundKey(state, RoundKey);
    for(int round = 1; round < NUM_ROUNDS; ++round)
    {
        aes256::SubBytes(state);
        aes256::ShiftRows(state);
        aes256::MixColumns(state);
        aes256::AESKeyScheduler(ExpandedKey, RoundKey, round);
        aes256::AddRoundKey(state, RoundKey);
    }
    /*  Last round. */
    aes256::SubBytes(state);
    aes256::ShiftRows(state);
    aes256::AESKeyScheduler(ExpandedKey, RoundKey, NUM_ROUNDS);
    aes256::AddRoundKey(state, RoundKey);
    /*  Copying encrypted password into output matrix. */
    for(int r = 0; r < NUM_BLOCKS; ++r)
    {
    	for(int c = 0; c < NUM_BLOCKS; ++c)
    		state_out[r][c] = state[r][c];
	}
}

void aes256::KeyExpansion(unsigned char ExpandedKey[NUM_BLOCKS][NUM_BLOCKS * (NUM_ROUNDS + 1)])
{
	...
	*/
	...
	for(int i = NUM_KEY, round = 1; i < NUM_BLOCKS * (NUM_ROUNDS + 1) ; ++i)
	{
		...
		if(NUM_KEY > 6 && i % NUM_KEY == 4)
			SubstituteWord(tmp);
		/*  XOR operations. */
		for(int j = 0; j < NUM_BLOCKS; ++j)
			ExpandedKey[j][i] = static_cast<unsigned char>(ExpandedKey[j][i - NUM_KEY] ^ tmp[j]);
	}
}

void aes256::AESKeyScheduler(const unsigned char ExpandedKey[NUM_BLOCKS][NUM_BLOCKS * (NUM_ROUNDS + 1)], 
                             unsigned char RoundKey[NUM_BLOCKS][NUM_BLOCKS], const int round)
{
	/*  Copying the round-th key into RoundKey matrix ready to encrypt state matrix. */
	for(int c = 0; c < NUM_BLOCKS; ++c)
	{
		for(int r = 0; r < NUM_BLOCKS; ++r)
			RoundKey[r][c] = ExpandedKey[r][(NUM_BLOCKS * round) + c];
	}
}

void aes256::SubBytes(unsigned char state[NUM_BLOCKS][NUM_BLOCKS])
{
	/*  See comments in line 140. */
	unsigned char c;
	/*  Rows. */
...
void aes256::ShiftRows(unsigned char state[NUM_BLOCKS][NUM_BLOCKS])
{
	unsigned char tmp;
	...
	for(int i = 1; i < NUM_BLOCKS; ++i)
	{
		...
		{             
			/* Third row, shift left by 2 positions. In a 4x4 matrix, shifting left
		...
		{
			/* Fourth row, shift left by 3 positions. In a 4x4 matrix, shifting left 
...
void aes256::MixColumns(unsigned char state[NUM_BLOCKS][NUM_BLOCKS])
{
	/*  Helper column */
	unsigned char tmp[NUM_BLOCKS];
...
void aes256::AddRoundKey(unsigned char state[NUM_BLOCKS][NUM_BLOCKS], const unsigned char RoundKey[NUM_BLOCKS][NUM_BLOCKS])
{
	for(int r = 0, i = 0; r < NUM_BLOCKS; ++r)
	{
		...
		*	     state                  RoundKey               state encrypted
		*/
		for(int c = 0; c < NUM_BLOCKS; ++c, ++i)
			state[r][c] = static_cast<unsigned char>(state[r][c] ^ RoundKey[r][c]); /*  XOR operation. */
	}
}
/*------------------------------------------------------------------------------------------------------------------------------*/
//  OTHER FUNCTIONS 
/*------------------------------------------------------------------------------------------------------------------------------*/
void PrintMatrix(const unsigned char matrix[NUM_BLOCKS][NUM_BLOCKS])
{
	std::cout << std::endl;
	for(int c = 0; c < NUM_BLOCKS; ++c)
	{
		for(int r = 0; r < NUM_BLOCKS; ++r)
			std::cout << static_cast<unsigned char>(matrix[r][c]);
	}
	std::cout << std::endl;
	...
	{
		for(int r = 0; r < NUM_BLOCKS; ++r)
			std::cout << std::hex << static_cast<int>(matrix[r][c]) << " ";
	}
	std::cout << std::endl;
}

void PrintExpandedMatrix(const unsigned char matrix[NUM_BLOCKS][NUM_BLOCKS * (NUM_ROUNDS + 1)])
{
	for(int r = 0; r < NUM_BLOCKS; ++r)
	{
		std::cout << "\n| ";
		for(int c = 0; c < NUM_BLOCKS * (NUM_ROUNDS + 1); ++c)
			std::cout << std::hex << static_cast<int>(matrix[r][c]) << " ";
		std::cout << "|";
	}
	std::cout << std::endl;
